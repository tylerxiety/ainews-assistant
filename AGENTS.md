...
- **AINews source**: Uses Latent Space Substack RSS with a private subscriber token (`feed?token=...`) that provides full content without cookie auth. Both `ainews` and `latent_space` sources share the same feed URL with inverse `titleFilter` regexes. The token was obtained from the Substack private podcast RSS feature but also works on the newsletter feed.
