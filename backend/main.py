...
@app.post("/backfill-chinese-section")
async def backfill_chinese_section(
    ...
    This is for showcase purposes - generates Chinese content for segments
    within a single section of the newsletter.
    ...
    try:
        ...
        if not groups:
            raise HTTPException(status_code=404, detail="No groups found for this issue")
        ...
        if section_start_order is None:
            raise HTTPException(
                status_code=404,
                detail=f"Section '{section_name}' not found. Available sections: {[g['label'] for g in groups if g.get('is_section_header')]}"
            )
        ...
        logger.error(f"Error in backfill_chinese_section: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal Server Error") from None
